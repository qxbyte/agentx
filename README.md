...
**基于 Spring AI 2.0 的个人本地智能体工作台（Claude Code / Codex 同构能力 + Web UI）。**

AgentX 是一个**运行在本机的 AI 工作台**：浏览器只是 UI 外壳，后端与你同机，因此模型可以真实地读写本地文件、执行 shell 命令、运行技能脚本——同时拥有 ChatGPT 风格的流式对话、RAG 知识库、Agent 编排与 MCP 双侧接入。技能（Skill）与插件（Plugin）体系完全兼容 Claude Code 格式，`obra/superpowers` 等现成插件市场可直接安装使用。

## 能力矩阵

| 能力 | 实现 | 说明 |
|---|---|---|
| 流式对话 | SSE 事件信封（12 种帧类型） | ChatGPT 风格 UI，思考过程/工具调用/审批/提问/引用溯源全程可视化；空会话开场输入框居中，浅色/深色/跟随系统主题 |
...
| Skill 技能 | 对标 Claude Code，格式兼容 | `~/.agentx/skills/` 本地目录化；`/` 斜杠命令自动展开 + 模型按描述自动触发（M2）；SKILL.md frontmatter 全量解析（`user-invocable` / `disable-model-invocation`）；L1 元数据 → L2 正文 → L3 资源（references/scripts）渐进披露，scripts 经审批本地真实执行 |
| Plugin 插件 | 完全兼容 Claude Code 插件/市场格式 | marketplace 添加（github/url/本地路径）→ 安装/更新/启停/卸载；插件贡献技能（`plugin:skill` 命名空间）、子代理（只读同步 + `dispatchAgent` 派遣）、MCP server（默认停用的信任边界） |
| 人机交互 | askUserQuestion + 审批门 + 任务清单 | 提问卡（单选/多选/预览选择器/其他自由输入/链式多问）；危险操作审批卡；两者**无限期等待**（虚拟线程挂起零成本，对标 Claude Code）；`updatePlan` 逐字移植 TodoWrite（content/activeForm 双形态、进度实时看板） |
...
| CodeAgent | 编码智能体（对标 Codex 桌面版） | 项目（工作区）体系 + Plan/Ask/Auto/**Bypass** 四模式（按会话记忆、轮内切换即时生效）+ 审批门；双根沙箱（工作区读写 + 家目录只读）；系统原生目录选择器（macOS Finder） |
...
- **一机一用户的本地 app**：不做多租户服务端。配置类数据（技能/插件/本地工具根目录）走 `~/.agentx/` 本地目录，不入库——换机迁移拷目录即可，与 Codex「账号只同步对话历史、不同步技能插件」同构。
...
| `agentx-tools` | 工具注册中心 + 内置工具 | `ToolRegistry` `ToolSource(SPI)` `@AgentTool`；`PlanTools`（TodoWrite）`AskUserQuestionTools` `WebTools`（webFetch/webSearch）示例工具 |
...
| `agentx-plugin` | 插件体系（Claude Code 兼容） | `PluginRegistry`（`~/.agentx/plugins/`）`MarketplaceService` `PluginService`（安装/更新/能力同步）`GitFetcher` `ManifestReader` `PluginSkillProvider` |
...
- **任务清单**：`updatePlan` 逐字移植 Claude Code TodoWrite（含全部示例）；content（祈使形）/activeForm（进行时形）双形态，输入框上方紧凑面板实时推进，折叠单行展示「正在……」。
...
- **四模式**（按会话记忆、轮内切换即时生效，切 Auto/Bypass 自动批准未决审批）：
  ...
  - `Bypass` 完全放行（对标 Claude Code bypassPermissions）：无审批、无路径边界，命令黑名单仅保留毁机级保护（rm -rf / 、fork bomb、mkfs、dd 写盘、shutdown）。
