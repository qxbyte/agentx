...
| 上下文与记忆 | 双轨制 + 自建记忆管线 | 业务表存完整历史；模型轨自管注入/回写——附件占位化（`readAttachment` 按需重读）、token 预算裁剪、工具轨迹摘要入忆、超阈值自动压缩 + `/compact` 手动压缩（摘要含任务清单恢复锚点）；输入框余量指示环实时显示用量；`~/.agentx/AGENTX.md`（用户级）+ 工作区 `AGENTX.md`（项目级）双级长期记忆，`saveMemory` 工具显式写入 |
| 工具调用 | 三级注册中心 | L1 代码级（`@AgentTool`+`@Tool`）· L2 MCP 远程 · L3 HTTP（预留 SPI）；启停运营开关、循环上限守卫、异常降级 |
| 本地天然工具 | 普通对话即有（本地 app 定位） | 文件读写/目录列举/grep/find/shell/webFetch/webSearch；家目录沙箱 + 写操作审批，Plan/Ask/Auto + Bypass 开关对普通对话同样生效 |
...
| CodeAgent | 编码智能体 | 项目（工作区）体系 + Plan/Ask/Auto 三档滑块 + **Bypass 盾牌开关**（确认弹窗进入，防误触；按会话记忆、轮内切换即时生效）+ 审批门；双根沙箱（工作区读写 + 家目录只读）；系统原生目录选择器（macOS Finder） |
...
| `agentx-chat` | 会话双轨 + 记忆管线 + 流式主链路 | `ChatStreamService`（SSE 无超时）`ModelMemoryService`（预算/压缩/回写）`MemoryFileService`（AGENTX.md）`AttachmentTools`/`MemoryTools`（readAttachment/saveMemory）`ConversationService` `QuestionController` |
...
模型上下文由自建记忆管线管理（业务历史与模型上下文双轨分离）：

- **附件占位化**：附件全文只在上传当轮注入，历史轮次入忆为占位符；模型需要细节时凭占位符里的 attachmentId 调 `readAttachment` 分页重读——大文档不再逐轮重发。
- **token 预算**：注入历史按估算裁剪（默认 32k），接近压缩阈值（默认 20k）自动触发滚动压缩；输入框右下角**余量指示环**实时显示用量（悬浮看明细，70%/90% 变色提醒）。
- **压缩**：自动或输入 `/compact` 手动——早期对话由模型压成结构化交接摘要（含任务清单恢复锚点），保留最近轮次原文；摘要失败时记忆保持原样。
- **工具轨迹摘要**：每轮的工具调用压成一行事实附在记忆里，防止模型跨轮重复探索（失败也记录）。
- **长期记忆（md 文件，用户可直接查看编辑）**：用户级 `~/.agentx/AGENTX.md` 每轮注入；项目级 `<工作区>/AGENTX.md` 随代码库走、可进版本管理；模型经 `saveMemory` 工具在对话中显式写入。
...
- **三档模式 + Bypass 开关**（按会话记忆、轮内切换即时生效，切 Auto/Bypass 自动批准未决审批）：
  - `Plan` 只读规划，写/执行工具不注册；
  - `Ask` 逐操作审批（默认，最安全）；
  - `Auto` 免审批直执行（沙箱边界与命令黑名单仍生效）；
  - `Bypass` 不是并列档位，而是 Auto 的「解除限制」态：停留在 Auto 时出现盾牌开关，
    经确认弹窗进入（三步门槛防误触，放行态红色标识）。解除路径边界与常规命令黑名单，
    仅保留毁机级保护（rm -rf / 、fork bomb、mkfs、dd 写盘、shutdown），随时点盾牌恢复。
...
- 仓内：`docs/database-schema.md`（全库表结构汇总）· `deploy/init-schema.sql`（Flyway 终态等价的全量初始化脚本，二选一使用）。
